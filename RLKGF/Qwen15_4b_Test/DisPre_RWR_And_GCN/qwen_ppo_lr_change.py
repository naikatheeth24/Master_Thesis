...
# PPO的优化类
class PPO_KG:
    def __init__(self, model, reward_model, tokenizer, sympton, disease, device, lr=8e-7, gamma=0.99, epsilon=0.2,
                 batch_size=16, update_freq=50, step_size=2, lr_decay=0.9963, min_lr=1e-7):
    ...
    def lr_lambda(self, step):
        ...
        定义学习率调度策略:
        """
        # 如果在初始化阶段 step 是0，避免直接更新学习率
        if step == 0:
            return 1.0  # 不改变学习率
        # 衰减阶段
        change_factor = (1- self.lr_change_ratio) ** (step // self.lr_step)  # 每2步降低0.01
        new_lr = self.lr * change_factor
        return max(new_lr, self.min_lr)  # 限制学习率
    ...
    def update(self, input_ids, generated_ids, old_log_probs, advantages):
        ...
        # 更新学习率调度器
        self.scheduler.step()  # 在每步优化后更新学习率
    ...
    def eval_step(self, goal_test, e=3):
        ...
        for i in range(e):
            ...
            print("success rate %.4f" % (dis_predict / episode))
