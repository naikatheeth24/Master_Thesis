...
# local_model_path = "./qwen-model"  # path where you saved the model locally
...
def generate_stream(
    ...
    temperature: float = 0.7,
    max_tokens: int = 1024,
    stream: bool = True,  # Ignored for local pipe
) -> str:
    print("Model is generating...\n")
    # outputs = pipe(prompt, max_new_tokens=max_tokens, temperature=temperature, do_sample=True)
    outputs = pipe(prompt, max_new_tokens=max_tokens, do_sample=False)


    generated = outputs[0]["generated_text"]
    # Only print and return the generated part (after the prompt)
...
def generate(
    ...
    return [
        pipe(prompt, max_new_tokens=100, temperature=temperature, do_sample=False)[0]["generated_text"]
...
# Play the Wordle game with a known secret word
# if __name__ == "__main__":
#     secret_word = "BRISK"
#     past_guesses = []

#     while len(past_guesses) < 6 and (not past_guesses or past_guesses[-1].guess != secret_word):
